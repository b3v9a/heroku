...
var Router = (function () {
    ...
    Router.prototype.start = function () {
        ...
        router.post('/comics/addcomic', function (req, res) {
            ...
            collection.find({ "comicTitle": comicTitle }, {}, function (e, docs) {
                if (e) {
                    ...
                    else if (String(req.session.passport.user) === 'undefined') {
        ...
        router.post('/comics/:id/view/all', function (req, res) {
            ...
            collection.findById(comicID, function (err, docs) {
                if (err) {
                    ...
                    collection.update({ _id: comicID }, { $set: { "modifiedDate": new Date() }
                    }, function (err, doc) {
                        if (err) {
        ...
        router.post('/register', function (req, res, next) {
            Account.register(new Account({ firstname: req.body.firstname, email: req.body.email, username: req.body.username, access: "" }), req.body.password, function (err, account) {
                if (err) {
        ...
        router.get('/view/category/:category', function (req, res) {
            ...
            var comicCollection = db.get('comiccollection');
            comicCollection.find({ $and: [{ comicCategory: category.name },
